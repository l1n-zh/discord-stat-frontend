...
function getDatasets(labelMapping, extractDataKey) {
    ...
    for (let message of query(messages, Object.values(filters)).sort(
        (a, b) => a.time - b.time
    )) {
    ...
    const dataSets = [];
    for (let [dataKey, data] of Object.entries(dataMapping).sort(
        ([key1], [key2]) => messageCounts[key2] - messageCounts[key1]
    )) {
        if (messageCounts[dataKey] == 0) break;
        dataSets.push({
            label:
                dataKey in labelMapping
                    ? labelMapping[dataKey]
                    : "Unknown " + dataKey,
            data: data,
        });
    }
...
function getTimeOfDayDatasets() {
    ...
    return getDatasets(labelMapping, (message) =>
...
function getAuthorDatasets() {
    ...
    for (let data of externalData.users) {
        labelMapping[data.value] =
            truncateString(data.title, 15) + " ( " + data.subtitle + " )";
    }
...
function submit() {
    ...
    chart.value.setDatasets(dataCache, topN.value);
    chart.value.setTopN(topN.value);
}
